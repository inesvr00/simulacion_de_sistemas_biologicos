...
def plot_histograma_kerma_dosis(df, nombre, N=None, E_0=None, min=None, max=None):
    ...
    if N:
        plt.hist(df['z'], bins=1000, weights=df['energia'] / (N * E_0 * ancho_contenedor), density=True, color='skyblue', edgecolor='black')
    else:
        plt.hist(df['z'], bins=1000, weights=df['energia'], color='skyblue', density=True, edgecolor='black')
    plt.xlabel('z')
...
def plot_kerma_y_dosis(df_dosis, df_kerma, N, E_0, min, max):
    def func_kerma(z, a, b):
        return a * np.exp(-b * z)

    if N:
        df_dosis = df_dosis[(df_dosis['z'] >= min) & (df_dosis['z'] <= max)]
        df_kerma = df_kerma[(df_kerma['z'] >= min) & (df_kerma['z'] <= max)]
    ...
    plt.plot(centro_contenedor, dosis_fit(centro_contenedor), 'b--', label='Ajuste Dosis (cúbico)')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'out/plots/dosis_kerma_fits_{min}_{max}.png')
